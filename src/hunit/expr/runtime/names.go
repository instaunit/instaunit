package runtime
// Name generation borrowed from Moby (Docker).
...
// DockerName generates a random name in the general style of Docker container names.
func DockerName() (string, string) {
  var l, r string
...
// Person names
var (
  // Most popular U.S. first names, based on Social Security registrations from 1916-2017,
...
// Obtain a random first name
func firstName() string {
  return first[rand.Intn(len(first))]
}

// Obtain a random last name
func lastName() string {
  return right[rand.Intn(len(right))]
}

// RandomName generates a random name consisting of n-1 person first names
// and a single Docker scientist last name, converted to title case.
func personName(n int) []string {
  v := make([]string, n)
  for i := 0; i < n; i++ {
    if i + 1 < n {
      v[i] = first[rand.Intn(len(first))]
    }else{
      v[i] = right[rand.Intn(len(right))]
    }
  }
  return v
}
