...
VERSION         ?= $(GITHASH)
RELEASE_DIR      = $(PWD)/target
RELEASE_TARGETS  = $(RELEASE_DIR)/$(GOOS)_$(GOARCH)
...
$(RELEASE_BINARY): $(SRC)
	(cd src && go build -ldflags="-X main.version=$(VERSION) -X main.githash=$(GITHASH)" -o $(RELEASE_BINARY) $(MAIN))
...
build_formula: build_release
	$(PWD)/build/update-formula -v $(VERSION) -o $(PWD)/formula/instaunit.rb $(RELEASE_PACKAGE)
